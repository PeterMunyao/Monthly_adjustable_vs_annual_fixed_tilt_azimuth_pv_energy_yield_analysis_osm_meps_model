...
# === NJAGU PLOTTING SCRIPT (Linked to Computational Results) ===
import matplotlib.pyplot as plt
import numpy as np

# --- Extract computed data from the previous code block ---
# Ensure these variables are already computed from the second code:
# monthly_results, monthly_fixed_energy, optimal_fixed_tilt, optimal_fixed_azimuth, months, month_names
...
# --- PLOT 1: Comparison of monthly optimal vs fixed optimal energy production ---
...
plt.bar(x_pos - bar_width/2, monthly_opt_energies, bar_width, 
        label='Monthly Optimal Tilt-azimuth Configuration', color=brown_color, alpha=0.8)
...
plt.xticks(x_pos, month_labels, rotation=1, fontsize=16)
...
for i, (opt_energy, fixed_energy) in enumerate(zip(monthly_opt_energies, fixed_opt_energies)):
    plt.text(i - bar_width/2, opt_energy + max(monthly_opt_energies)*0.01, f'{opt_energy:.0f}', 
             ha='center', va='bottom', fontsize=10, fontfamily='Garamond', fontweight='bold')  
    plt.text(i + bar_width/2, fixed_energy + max(fixed_opt_energies)*0.01, f'{fixed_energy:.0f}', 
             ha='center', va='bottom', fontsize=10, fontfamily='Garamond', fontweight='bold')  

# --- Place legend below the plot ---
plt.legend(fontsize=16, framealpha=0.9, facecolor='white', edgecolor='gray', 
           loc='upper center', bbox_to_anchor=(0.5, -0.12), ncol=2)

plt.tight_layout()
plt.savefig("NJAGU_MONTHLY_vs_FIXED_OPTIMAL_COMPARISON_size2.pdf", format="pdf", bbox_inches="tight", dpi=300)
plt.show()
...
def plot_corrected_analysis(monthly_results, monthly_fixed_energy, months, month_names):
    monthly_optimal = np.array([monthly_results[m]['max_energy'] for m in months if m in monthly_results])
    fixed_optimal = np.array([monthly_fixed_energy[m] for m in months if m in monthly_results])
    differences = monthly_optimal - fixed_optimal
    monthly_labels = [month_names[m-1] for m in months if m in monthly_results]
    monthly_tilts = [monthly_results[m]['optimal_tilt'] for m in months if m in monthly_results]
    monthly_azimuths = [monthly_results[m]['optimal_azimuth'] for m in months if m in monthly_results]
    
    # Plot 2b: Monthly energy gains
    plt.figure(figsize=(14, 7))
    plt.bar(monthly_labels, differences, color='#2E8B57', alpha=0.7, width=0.5)
    ...
    for i, diff in enumerate(differences):
        plt.text(i, diff + max(differences)*0.01, f'{diff:.1f}', 
                ha='center', va='bottom', fontsize=14, fontfamily='Garamond', fontweight='bold')

    plt.tight_layout()
    plt.savefig("STATISTICAL_ANALYSIS_ENERGY_GAINS_size2.pdf", format="pdf", bbox_inches="tight", dpi=300)
    plt.show()

    return monthly_optimal, fixed_optimal, differences, monthly_labels, monthly_tilts, monthly_azimuths

monthly_optimal, fixed_optimal, differences, monthly_labels, monthly_tilts, monthly_azimuths = \
    plot_corrected_analysis(monthly_results, monthly_fixed_energy, months, month_names)


# === PLOT 3: MERGED — Monthly optimal tilt and azimuth angles ===
...
line1 = ax1.plot(monthly_labels, monthly_tilts, 'o-', color=color1, linewidth=2.5, markersize=14, 
                markerfacecolor=color1, markeredgecolor='white', markeredgewidth=3, 
                label='Optimal Tilt Angle')
ax1.tick_params(axis='y', labelcolor=color1, labelsize=18)
ax1.tick_params(axis='x', labelsize=16, rotation=1)
ax1.set_ylim(0, 90)
ax1.set_xlim(-0.5, len(monthly_labels)-0.5)

# Secondary axis for azimuth
ax2 = ax1.twinx()
ax2.set_facecolor('#f8f9fa')
color2 = '#d62728'
ax2.set_ylabel('Optimal Azimuth Angle (°)', color=color2, fontsize=20, fontfamily='Garamond', fontweight='bold')
line2 = ax2.plot(monthly_labels, monthly_azimuths, 's-', color=color2, linewidth=2.5, markersize=14,
                markerfacecolor=color2, markeredgecolor='white', markeredgewidth=3,
                label='Optimal Azimuth Angle')
ax2.tick_params(axis='y', labelcolor=color2, labelsize=18)
ax2.set_ylim(0, 360)

# Grid and ticks
ax1.grid(True, alpha=0.5, linestyle='-', linewidth=1.0, which='major')
...
legend = ax1.legend(lines, labels, loc='upper left', fontsize=15, framealpha=0.7,
                   facecolor='white', edgecolor='gray', frameon=True)
legend.get_frame().set_linewidth(2)

# Annotate all points
for i, (tilt, azimuth) in enumerate(zip(monthly_tilts, monthly_azimuths)):
    ax1.annotate(f'{tilt}°', (i, tilt), textcoords="offset points", 
                xytext=(0,10), ha='center', fontsize=11, fontfamily='Garamond',
                bbox=dict(boxstyle="round,pad=0.1", facecolor='lightblue', alpha=0.55))
    ax2.annotate(f'{azimuth}°', (i, azimuth), textcoords="offset points", 
                xytext=(0,-10), ha='center', fontsize=11, fontfamily='Garamond',
                bbox=dict(boxstyle="round,pad=0.1", facecolor='lightcoral', alpha=0.55))

plt.tight_layout()
plt.savefig("NJAGU_MONTHLY_OPTIMAL_ANGLES_COMBINED_size2.pdf", format="pdf", 
            bbox_inches='tight', facecolor='#f8f9fa', dpi=300)
plt.show()
